...
def _embed_text(entity: ExtractedEntity) -> str:
    ...
    На прогоне GigaChat-2-Max было видно, что descriptor шумит и тянет
    сходство в сторону «похожих по тематике, но разных» сущностей: «старик»
    и «дед» (синонимы одного персонажа) получали разные descriptor'ы и не
    сматчивались в резолвере. Имя сопоставляется с ``wiki_section.embedding``,
    в котором уже сидят и название страницы, и контент — этого достаточно
    как сигнала; descriptor больше вредит, чем помогает.
    Statements не включаем по той же причине (это было и раньше).
    """
    return entity.name
